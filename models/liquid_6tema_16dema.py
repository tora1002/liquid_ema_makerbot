...
class Liquid6tema16dema(Base):
    
    """
    Liquid6tema16demaModel
    """
    __tablename__ = "liquid_6tema_16dema"
    ...
    def get_limit_record_order_desc(session, limit_num):
        return session.query(Liquid6tema16dema).order_by(desc(Liquid6tema16dema.id)).limit(limit_num).all()
